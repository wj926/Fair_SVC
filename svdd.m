function model = svdd(input,options)
...
%  figure; hold on; 
%  ppatterns(input); 
%  ppatterns(input(:,model.sv_ind),'ro',12);   % plot SV
%  ppatterns(input(:,model.bsv_ind),'bo',12);  % plot BSV
...
% Modified by Kyu-Hwan Jung at April 29, 2010.
%
% * The source code is available under the GNU LESSER GENERAL PUBLIC
% LICENSE, version 2.1. 
%==========================================================================

disp('Step 1 : Training Support Function by SVDD ...')
...
[dim,num_data] = size(input);

% set up QP problem
%-----------------------------------------

K = kernel(input, options.ker, options.arg );
f = -diag(K);
H=2*K;
b=options.C;
I=[1:1:num_data]';
...
% setup model
%---------------------
model.sv.X= input(:,inx);   % including both of SV and BSV (bounded support vectors)
model.sv.inx = inx;
model.nsv = length(inx);
model.options=options;
model.stat = stat;
model.fun = 'kradius';
model.r = max(kradius(input(:,model.sv_ind),model));

disp('Training Completed !');

training_time_svdd=toc
